...
class LoginService implements ISignInServiceInterface {
  ...
  public async signIn(data: LoginDTO): Promise<any> {
    ...
    let isPasswordValid = await this.passwordService.checkPassword(
      password,
      userData.password
    );
    if (!isPasswordValid) {
    ...
    let acessToken = this.tokenManger.generateToken(
      { userEmail: email, userName, userId },
      24 * 60 * 60 * 1000
    );
    let refreshToken = this.tokenManger.generateToken(
      userData._id,
      24 * 60 * 60 * 1000
    );
  ...
  public async signUp(data: LoginDTO): Promise<any> {
    ...
    let updateFrnd = await frndRepository.updateList(
      { user2Email: email },
      { user2Id: uid }
    );
  ...
  public async getuserInfo(data: any): Promise<any> {
    let friendRequestsData = await this.friendsService.getFriendRequests(
      data?.userId
    );
  ...
  public async googleLogin(data: any): Promise<any> {
    ...
    let { userEmail, userName, userId } = userData;
    ...
    return { refreshToken, acessToken, message: "Google login sucessfull" };
  }
}
