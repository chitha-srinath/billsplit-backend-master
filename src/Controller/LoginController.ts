...
export interface ILoginControllerInterface {
  ...
  handleGoogleLogin(req: Request, res: Response): Promise<any>;
}
...
class LoginController implements ILoginControllerInterface {
  ...
  public async getuser(
    ...
    try {
      ...
      const result: ApiResponse = HandleResponse.handleResponse(
        true,
        200,
        "user fetchied sucessfully",
        serviceInfo
      );
      return res.send(result);
    } catch (err) {
      console.log("------err from fetch user details ----", err);
      const result: ApiResponse = HandleResponse.handleResponse(
        false,
        500,
        "fetch user failed",
  ...
  public async handleGoogleLogin(req: Request, res: Response): Promise<any> {
    try {
      console.log(req.cookies);
      const serviceInfo = await this.loginService.googleLogin(req.user);
      let result: ApiResponse;
